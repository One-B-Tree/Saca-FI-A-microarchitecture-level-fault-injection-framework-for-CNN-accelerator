...
def create_csv_info(config_info=[],mode='lenet',data_flow='ws',layer=1):
    test_fc1_24x24 = [27, 37, 512, 27, 37, 512, 1, 24, 1]
    test_yolo_tiny_conv1_24x24 = [418, 418, 3, 3, 3, 16, 1, 24, 1]
    test_mdnet_conv1_24x24 =config_info
    Loc=data_flow
    ...
    # Model parameters
    dimensions_h = param[7] #32 #16
    dimensions_w=param[8]
    word_sz = param[9]
    ...
    name1='scale_sim/RS/'+Loc+'/'+ mode + '/'
    name2='sram_read'+str(layer)+'pe'+str(dimensions_h)+'.csv'
    rename_sram_read=name1+name2
    sram_read_trace = "z_sram_read.csv"
    sram_write_trace  = "z_sram_write.csv"

    dram_filter_read_trace = "z_dram_filt_read.csv"
    dram_ifmap_read_trace  = "z_dram_ifamp_read.csv"
    dram_write_trace = "z_dram_write.csv"

    bw_numbers, detailed_log, util, sram_cycles = gen_all_traces(
        array_h = dimensions_h,
        array_w = dimensions_w,
    ...
    shutil.copy(sram_read_trace,rename_sram_read)

def test(data_flow='ws'):
    test_mdnet_conv1_24x24 = [1, 1, 4096, 1, 1, 1000, 1, 256, 256, 1]
    param = test_mdnet_conv1_24x24
    # The parameters for 1st layer of yolo_tiny
    ifmap_h = param[0]
    ifmap_w = param[1]
